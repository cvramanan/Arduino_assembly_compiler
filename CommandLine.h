...
const char *multiplicationCommandToken = "mul";
const char *divisionCommandToken = "div";
const char *moduloCommandToken = "mod";
const char *clearscrCommandToken = "clear";
const char *builtinledCommandToken = "led";
...
DoMyCommand(char * commandLine) {
  //  print2("\nCommand: ", commandLine);
  int result;
//Serial.print("ECE@32-bit_machine:~$");
  ...
  if (strcmp(ptrToCommandName, addCommandToken) == 0) {                   //Modify here
    ...
    else if(strcmp(ptrToCommandName,multiplicationCommandToken ) == 0)
    {
       result = multiplicationCommand();                                       //K&R string.h  pg. 251
      print1(result);
    }
       else if(strcmp(ptrToCommandName,divisionCommandToken ) == 0)
    {
       result = divisionCommand();                                       //K&R string.h  pg. 251
      print1(result);
    }
    else if(strcmp(ptrToCommandName,moduloCommandToken ) == 0)
    {
       result = moduloCommand();                                       //K&R string.h  pg. 251
      print1(result);
    }
      else if(strcmp(ptrToCommandName,clearscrCommandToken ) == 0)
    {
      for(int i =0;i<100;i++)                                       //K&R string.h  pg. 251
      print1(" ");
      Serial<<byte(CR);
     // Serial.println("hi");
    }
    else if(strcmp(ptrToCommandName,builtinledCommandToken ) == 0)
    {
       result = moduloCommand();                                       //K&R string.h  pg. 251
     if(result == 1)
      print1("Led is on");
